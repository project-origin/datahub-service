...
Revision ID: 85eab8825f75
Revises: 
Create Date: 2020-04-23 18:31:22.987400
...
def upgrade():
    ...
    sa.Column('type', sa.Enum('PRODUCTION', 'CONSUMPTION', name='measurementtype'), nullable=False),
    ...
    sa.Column('ledger_extended_key', sa.String(), nullable=True),
    ...
    op.create_index(op.f('ix_meteringpoint_sub'), 'meteringpoint', ['sub'], unique=False)
    op.create_index(op.f('ix_meteringpoint_type'), 'meteringpoint', ['type'], unique=False)
    op.create_table('webhook_subscription',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('event', sa.Enum('ON_GGOS_ISSUED', 'ON_METERINGPOINTS_AVAILABLE', name='event'), nullable=False),
...
def downgrade():
    ...
    op.drop_index(op.f('ix_meteringpoint_sub'), table_name='meteringpoint')
